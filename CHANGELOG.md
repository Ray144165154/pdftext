...
- **xref 表只认 LF 换行**：规范 7.2.3 允许 `\r`、`\n`、`\r\n` 三种行结束符，
  但早期实现用 `buf.find(b"\n", pos)` 找行尾。遇到整张 xref 表用**裸 `\r`**
  换行的文件时，"子段标题行"会被一直读到第一条条目末尾，字段数从 2 变成 5，
  导致整张表解析失败、`trailer` 也随之读不到，最终降级为全盘扫描重建——
  **一份完好的文件被误判成"交叉引用表损坏"**，并向用户打出错误警告。

  现按行读取（不再假设条目宽度固定 20 字节），并同时接受三种换行符。

  该问题由一份 **11 MB / 382 页的真实扫描版 PDF**（PDF 1.4）测出，
  修复后 `repaired` 由 `True` 变为 `False`。已加入 4 个回归测试，
  覆盖 `\n`、`\r`、`\r\n` 三种风格并验证三者解析结果完全一致。
