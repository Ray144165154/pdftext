...
`pdftext` 就是干这个的：**2699 行、纯 Python、零依赖**，实现了 PDF 文本提取真正需要的那部分规范。
...
```
Ran 152 tests in 0.071s

OK
```

152 个测试，覆盖词法/语法、解码器、CMap、内容流状态机、版面还原、
...
│   ├── extract.py     顶层提取 API
│   └── cli.py         命令行接口
├── tests/             152 个测试（含纯 Python 的 PDF 生成器）
├── tools/             零依赖检查脚本
├── docs/              限制说明
